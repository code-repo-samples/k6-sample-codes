...
const PROFILE_LABEL = (args.profile || process.env.PROFILE || 'all').toUpperCase();
...
// browser_web_vital_fcp, etc.) that there's no collision risk.
...
function ensureUrlBrowser(url) {
  if (!perUrlBrowser[url]) {
    perUrlBrowser[url] = { fcp: [], lcp: [], cls: [], tti: [], errors: new Map(), samples: 0 };
  }
...
for (const line of lines) {
  ...
  if (name.startsWith('browser_')) {
    sawBrowserData = true;
    const url = tags.url_label || tags.url || tags.name || '';
    if (name === 'browser_dom_errors') {
    ...
    if (!url) continue;
    const b = ensureUrlBrowser(url);
    if (name === 'browser_web_vital_fcp') { b.fcp.push(data.value); b.samples++; }
    else if (name === 'browser_web_vital_lcp') b.lcp.push(data.value);
    else if (name === 'browser_web_vital_cls') b.cls.push(data.value);
    else if (name === 'browser_web_vital_ttfb') b.tti.push(data.value);
    else if (name === 'browser_web_vital_tti') b.tti.push(data.value);
    continue;
  }
...
if (sawProtocolData) {
  ...
  const resourceAccordionRows = urls.map((u) => {
    ...
    const matches = Object.entries(discovery).filter(([pageKey, info]) => matchDiscoveryForUrl(pageKey, info));
  ...
  const urlErrorRows = urls.map((u) => errorRowsHtml(perUrl[u].errors, u)).join('\n');
  ...
  <section>
    <h2>User Behavior Mix <span class="sub">flag-driven, applied per iteration</span></h2>
    <div class="mix-row">
      <div class="mix-card"><div class="label">Cache disabled (new users)</div>
      ...
      <div class="mix-card"><div class="label">Cache enabled (returning users)</div>
      ...
      <div class="mix-card"><div class="label">Iterations that fetched embedded resources</div>
        <div class="bar-track"><div class="bar-fill" style="width:${(embeddedAttempts / embeddedBase * 100).toFixed(0)}%"></div></div>
        <div class="value num">${pctFmt(embeddedAttempts / embeddedBase * 100)}</div></div>
      <div class="mix-card"><div class="label">Page-only iterations</div>
        <div class="bar-track"><div class="bar-fill" style="width:${(100 - (embeddedAttempts / embeddedBase * 100)).toFixed(0)}%"></div></div>
        <div class="value num">${pctFmt(100 - (embeddedAttempts / embeddedBase * 100))}</div></div>
    </div>
  </section>`;
} else if (PROTOCOL_FILE) {
...
if (sawBrowserData) {
  ...
  const rows = browserUrls.map((u) => {
    const b = perUrlBrowser[u];
    const lcpP95 = pct(b.lcp, 95);
    const fcpP95 = pct(b.fcp, 95);
    const lcpPill = !b.lcp.length ? '' : lcpP95 < LCP_THRESHOLD ? '<span class="pill ok">pass</span>' : '<span class="pill bad">fail</span>';
    const fcpPill = !b.fcp.length ? '' : fcpP95 < FCP_THRESHOLD ? '<span class="pill ok">pass</span>' : '<span class="pill bad">fail</span>';
    const errCount = [...b.errors.values()].reduce((a, c) => a + c, 0);
    return `<tr>
      <td>${esc(u)}</td>
      <td class="num">${b.samples}</td>
      <td class="num">${avgOrDash(b.fcp)}</td><td class="num">${pctOrDash(b.fcp, 95)} ${fcpPill}</td>
      <td class="num">${avgOrDash(b.lcp)}</td><td class="num">${pctOrDash(b.lcp, 95)} ${lcpPill}</td>
      <td class="num">${b.cls.length ? f3(avg(b.cls)) : '–'}</td>
      <td class="num">${avgOrDash(b.tti)}</td>
      <td class="num">${errCount}</td>
    </tr>`;
  }).join('\n');

  const browserErrRows = browserUrls.map((u) => {
    const b = perUrlBrowser[u];
    if (!b.errors.size) return '';
    return [...b.errors.entries()].map(([type, count]) =>
      `<tr><td>${esc(u)}</td><td>${esc(type)}</td><td class="num">${count}</td></tr>`
    ).join('\n');
  }).filter(Boolean).join('\n') || `<tr><td colspan="3" style="text-align:center;color:#6B7280">No browser errors recorded</td></tr>`;

  browserSectionHtml = `
  <section class="section-heading"><h1>Browser-Level Results <span class="scope">WEB VITALS</span></h1>
    <p class="note">From the k6/browser Web Vitals script — real Chromium rendering metrics, gathered concurrently while the protocol-level script (if run) put the infrastructure under load.</p>
  </section>

  <section>
    <h2>Core Web Vitals <span class="sub">per page &mdash; thresholds: LCP p95 &lt; 2500ms, FCP p95 &lt; 1500ms</span></h2>
    <table class="card"><thead><tr>
      <th>URL</th><th>Samples</th><th>FCP Avg</th><th>FCP p95</th><th>LCP Avg</th><th>LCP p95</th><th>CLS Avg</th><th>TTI Avg</th><th>DOM Errors</th>
    </tr></thead><tbody>${rows}</tbody></table>
  </section>

  <section>
    <h2>Browser Errors <span class="sub">render collapses, navigation timeouts</span></h2>
    <table class="card"><thead><tr>
      <th style="text-align:left">URL</th><th style="text-align:left">Error Type</th><th>Count</th>
    </tr></thead><tbody>${browserErrRows}</tbody></table>
  </section>`;
} else if (BROWSER_FILE) {
