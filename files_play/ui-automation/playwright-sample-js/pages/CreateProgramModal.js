...
class CreateProgramModal {
  ...
  /**
   * Fills the whole modal from the test-data/test-config.json "program" block,
   * with a unique name so repeated runs don't collide. Returns the name used,
   * since that's what you look the program up by afterward.
   */
  async fillFromConfig(programConfig, { uniqueName } = {}) {
    const name = uniqueName || `${programConfig.namePrefix}-${Date.now()}`;

    await this.fillProgramDetails({
      name,
      description: programConfig.description,
      startDate: programConfig.startDate,
      endDate: programConfig.endDate,
      activeMarket: programConfig.activeMarket,
      ownerGroup: programConfig.ownerGroup,
    });

    await this.setRedemptionRules(
      programConfig.maxValidAttempts,
      programConfig.redemptionPer,
      programConfig.redemptionPeriod
    );

    await this.selectCharacterSet(programConfig.characterSet || {});
